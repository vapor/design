...
| `partials/pagination.leaf` | Generic pagination (port of the Publish `Pagination`). Render with `#extend("partials/pagination", paginationContext)` where the context has `hasMultiplePages`, `hasPrevious`/`prevURL`, `hasNext`/`nextURL`, `currentPage`, `totalPages`, and `links[]` (each `isEllipsis`/`isCurrent`/`page`/`url`). Used by the website team page and the blog. **Note:** because the context is scoped to the pagination object, built-in `strings.*` are NOT available inside — the "Previous"/"Next" labels are literal. |
